...
contract("Vurnerable", (accounts) => {
  ...
  context("testgroup - security tests - description...", () => {
    ...
    before(async () => {
      /* before tests */
      const newVurnerable = await Vurnerable.new();
    });
    ...
    it("fails on initialize ...", async () => {
      return assertRevert(async () => {
        await newVurnerable.initialize();
      });
    });

    it("checks if method returns true", async () => {
      assert.isTrue(await newVurnerable.thisMethodShouldReturnTrue());
    });
  });
});
